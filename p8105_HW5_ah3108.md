...
The obersvations for subjects in the Experiment group increased across
time. By the 6th week almost all subjects in the experiment group had
higher observations than those in the control group. By the end of the
study period, in the 8th week, all subjects in the experiment group had
...
sim_results_small =
  ...
  mutate(
    output_list = map(.x = beta1, ~rerun(100, sim_data(beta1 = .x))),
...
prob3_plot %>% 
  ggplot(aes(x = beta1, y = prop_reject_H0)) + 
...
prob3_plot %>% 
  ...
  geom_bar(stat = "identity") +
    labs(
    title = "Reject H0 based on beta 1",
    x = "Beta 1",
    y = "% Reject HO")
```
